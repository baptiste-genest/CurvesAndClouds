...
void cnc::graphics::Mesh::computeNormals()
{
    ...
    for (index f = 0;f<(index)TRIANGLE_COUNT;f++){
        for (index i = 0;i<3;i++){
            index id = m_indexArray[f*3+i];
            m_normals[id] += face_normals[f]/(face_areas[f]+1e-5);
        }
    }
...
{
    index id = f*3;
    const vec3& a = m_vertices[m_indexArray[id]];
    const vec3& b = m_vertices[m_indexArray[id+1]];
    const vec3& c = m_vertices[m_indexArray[id+2]];
